...
from opencompass.utils import (Menu, build_dataset_from_cfg,
                               ...
                               model_abbr_from_cfg)
time_str = '20240523_071729'
default_config = f'/app/code/llm_benchmark/outputs/default/{time_str}/configs/{time_str}.py'

def parse_args():
    parser = argparse.ArgumentParser(
        description='View generated prompts based on datasets (and models)')
    parser.add_argument('--config', help='Train config file path', default=default_config)
...
def print_prompts(model_cfg, dataset_cfg, count=1):
    ...
    if model_cfg:
        ...
        # force set model_cfg['tokenizer_only'] True
    ...
    for idx in range(min(count, len(ice_idx_list))):
        ...
        if 'PPLInferencer' in infer_cfg.inferencer.type or 'LLInferencer' in infer_cfg.inferencer.type:
